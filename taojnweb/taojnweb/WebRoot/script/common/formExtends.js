 ...
 * @return
 */
function SimpleFormNoConfirm(formId, actionName) {
	this.formId = formId;
		...
		checkForm.validate( {ignore:"",
			submitHandler : function(form) {
				if (confirm("确定" + actionName + "?")) {
					...
					options.beforeSend=function(XMLHttpRequest){disabledForm(formId);WaitDialog.show("正在提交...")};
					options.complete=function(XMLHttpRequest){enabledForm(formId);WaitDialog.hide();};
					options.dataType = "text";
					var ajaxForm = $(form);
					ajaxForm.ajaxSubmit(options);
				}
...
function BackUrlFormNoConfirm(formId, url) {
	this.formId = formId;
	this.result = {};
	this.url = url;
	this.init = function() {
 ...
 * @return
 */
function BackMessageForm(formId, actionName) {
	this.formId = formId;
	this.actionName = actionName;
	this.result = {};
	this.init = function() {
			...
			submitHandler : function(form) {
				if (confirm("确定" + actionName + "?")) {
					...
					options.success = function(rInfo) {
						...
						checkForm._form.result = obj;
						if (obj.flag == "success") {
							window.location.href =obj.message[0];
						}else
						{
							alert(obj.message[0]);
						}
...
var WaitDialog= new function (){
	...
	this.show = function(message){
		var style="background:#ffffff;position:absolute;height:80px;width:300px;top:50%;left:50%;margin-left:-150px;margin-top:-150px;text-align:center;padding:7px 0 0 0;font:bold 20px Arial, Helvetica, sans-serif;";
		var html="<div id=\"loading\" style=\""+style+"\">"+message+"..<img src=\""+applicationContent.baseUrl+"/style/m/images/waiting.gif\" alt=\"wait...\" /></div>";
		this.div =$(html);
