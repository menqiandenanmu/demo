...
validator={
	...
	ajax: "this.doajax(errindex,formId)",
	bigger : "this.checkBiggerThan(value)",// add by Mr.zhaoxs
	...
	checkBiggerThan:function(value){// add by Mr.zhaoxs
		var to = this['element'].attr('to');
		return Number(value)>Number(jQuery('input[name="'+to+'"]').eq(0).val());
	},
	checkRange : function(value){
		value = value|0;
		var minval=this['element'].attr('minval') || Number.MIN_VALUE;
		var maxval=this['element'].attr('maxval') || Number.MAX_VALUE;
	...
	doajax : function(value,formId) {	
		var form=jQuery("#"+formId);
		var element = this['element'];
		var errindex = this['errindex'];
		var url=this['element'].attr('url');
		var remote=this['element'].attr('remote');
		...
		if(url==null&& remote==null) {
			alert('no url or remote attr');
			return false ;
		}
		if(url!=null && remote!=null) {
			alert('url and remote there is only one');
			return false ;
		}
		var s;
		if(url!=null){
			if(url=="" || url==undefined){alert("please don't forget attribute value of url");return false ;}
			if(url.indexOf('?')>-1){
		    ...
		    s = $.ajax({//only submit current element
				type: method,
				url: url,
				data: {},
				cache: false,
		...
		}else if(remote!=null){//ajax form submit
			if(remote=="" || remote==undefined){alert("please don't forget attribute value of remote");return false ;}
			var option={};
			option.url=remote;
			option.async=false;
...
validator.showErr=function (element, errindex){
	...
	element.parent('*').find('.'+this['validTip']).remove();//add by Mr.zhaoxs
	if(msgid.length>0) {
		msgid.removeClass(this['validTip']).addClass(this['errorTip']).html(errmsg);
	} else {
		jQuery("<span class='"+this['errorTip']+"'>&nbsp;&nbsp;&nbsp;</span>").html(errmsg).insertAfter(element);//validate failed tips
	}
	return false ;
}
...
validator.checkajax = function(element, datatype, errindex ,formId) {
...
validator.check=function(obj,form){
	...
	if(obj.attr('require')!="true" && value=="") {
		...
		if(msgid.length>0) {
     		...
     		jQuery('<span class="'+validator.validTip+'">&nbsp;&nbsp;&nbsp;&nbsp;</span>').insertAfter(obj);//validate success tips
		}	
		return true;
	}
	...
	jQuery.each(datatypes,function(index,type){
		...
		if(type=='ajax')//ajax validate 
			return isValid = validator.checkajax(obj, type, index ,form.attr("id"));
 ...
 /**
  * submitType 0 : form normal submit,1 : form ajax submit
  * fckFlag true:page have fckeditor,false:no
  * method call back method
  */
jQuery.fn.checkForm = function(submitType,fckFlag,method){
	var form=jQuery(this);
	var elements = form.find(':input[require]');
	elements.blur(function(index){
		return validator.check(jQuery(this),form);
	});
	...
	form.submit(function(){
		...
		elements.each(function(i){
			if(validator.check(jQuery(this),form)==false){
				isValid  = false;
				errIndex[n++]=i;
			};
		});
		...
		if(submitType==0||submitType==null){//not ajax submit
			return isValid;
		}else if(submitType==1){//ajax submit,callback have tips
			if(fckFlag==true){
				if (FCKeditorAPI){
