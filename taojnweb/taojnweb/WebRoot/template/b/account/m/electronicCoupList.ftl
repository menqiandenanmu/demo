...
<script language="JavaScript">
    ...
    function deleteMessage(){
		 ...
		 function updateMessage(){
		 	...
		 	common.doPostData('${base}/manage/account/electronicCoup!useAll.htm','确定充值吗?', options);
...
<!--列表正文开始-->
    <div class="page-main">
        <div class="page-item page-title">电子券管理</div>
        <!--索搜区域-->
		<form action="${base}/manage/account/electronicCoup.htm" id="searchForm" method="get" >
        <div class="page-item search-panel">
 		  <ul class="search-list clearfix">
			...
			<li><span class="lab-title">审核人:</span>
			...
			<li><span class="lab-title">来源:</span>
				...
				<select name="electronicCoup.resource" style="width:180px;">
					...
					<#list sysDictDetails as item>
						<option	value=${item.dictDetailCode} <#if electronicCoup.resource?exists&& electronicCoup.resource==item.dictDetailCode >selected</#if>>${item.dictDetailName}</option>
					</#list>
				</select>
			</span>
			</li>
       ...
       <input type="submit" value="搜索" class="btn btn-small bg-blue"/>&nbsp;&nbsp;&nbsp;&nbsp;
           ...
           <#--input type="button" class="btn btn-small bg-blue" name="button3" id="button3" onclick="updateMessage();" value="充值选中" />&nbsp;&nbsp;&nbsp;&nbsp;-->
           
				<input type="button" class="btn btn-small bg-blue" onClick="location.href='${base}/manage/account/electronicCoup!electronicCoupExport.htm?${queryString}'" value="导出">
			&nbsp;&nbsp;&nbsp;&nbsp;
            <a href="javascript:;" onclick="location.href='${base}/manage/account/electronicCoup!add.htm'" class="btn btn-small bg-blue" id="add-btn">+新增</a>
        </div>
        	</form>
        <!--列表操作区域结束-->
 	<!--列表-->
        ...
        <form action="${base}/manage/account/electronicCoup!delAll.htm" id="messageForm" name="messageForm" method="post" >
            <table class="list-table" id="listTable">
                <thead>
                    <tr>
                    <td width="2%" align="left" class="table_title"><input type="checkbox" name="allCheck" value="" onchange="changeBox();" /></td>
                     <th class="w5" width="80px;">序号</th>
					  <th class="w10" width="80px;">客户名称</th>
					   <th class="w10" width="80px;">充值来源</th>
					   <th class="w10" width="80px;">电子券编号</th>
					  <th class="w8" width="80px;">金额</th>
					  ...
					  <th class="w10" width="80px;">充值时间</th>
					   <th class="w5" width="280px;">审核人</th>
	    	...
	    	<#list pages.result as res>
				<#assign flag=1>
			    <tr  id="${res.id}">
			    <td width="2%" align="left" ><input type="checkbox" name="messageIds" value="${res.id?if_exists}" /></td>
			      <td align="center">${res_index+1}</td>
			      <td align="center">${res.accountName?default("")}</td>
			        <td align="center">${res.eletName!}</td>
			      <td align="center">${res.eletCode?default("")}</td>
			      ...
			      <td align="center">
			       <#if res.auditSyatus==0>
			      	<a href="${base}/manage/account/electronicCoup!edit.htm?id=${res.id}">编辑</a>
		      		&nbsp;<a href="javascript:common.doPost('${base}/manage/account/electronicCoup!del.htm?id=${res.id}','确定删除吗?');">删除</a>
		      		</#if>
		      		<a href="${base}/manage/account/electronicCoup!info.htm?id=${res.id}">详情</a>
			      </td>
			    </tr>
			</#list>
			<#if flag=0>
