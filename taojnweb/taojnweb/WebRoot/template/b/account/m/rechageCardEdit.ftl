...
<script language="JavaScript">
    $(document).ready(function(){
     	var fm=new BackUrlForm("validateForm","修改充值卡","${base}/manage/account/rechageCard.htm");
    });
</script>
</head>
<body>
  <!--表单正文开始-->
            <div class="page-main">
                <div class="page-item page-title">充值卡修改</div>
                <div class="page-item form-panel">
                   <form action="${base}/manage/account/rechageCard!update.htm" id="validateForm" method="post" >
                    <input name="id" hidden class="text" size="30" value="${rechageCard.id!}"  validate="{required:true}" />
                        <ul>
                           ...
                           <li><span class="lab-title">充值卡卡号</span>
                             ...
                             <li><span class="lab-title">生成年度</span>
                                <div class="form-item">
                                   <input name="rechageCard.createYear"  class="text" size="30" value="${rechageCard.createYear!}" onclick="WdatePicker({isShowClear:false,readOnly:true,skin:'blue',dateFmt:'yyyy'})" validate="{required:true,maxlength:20}" />
                                </div>
                            </li>
                             <li><span class="lab-title">失效时间</span>
                                <div class="form-item">
                                    <input name="rechageCard.failureTime"  class="text" value="${rechageCard.failureTime?string("yyyy-MM-dd")!}" size="30" onclick="WdatePicker({isShowClear:false,readOnly:true,skin:'blue'})"  validate="{required:true}"/>
                                </div>
                            </li>
