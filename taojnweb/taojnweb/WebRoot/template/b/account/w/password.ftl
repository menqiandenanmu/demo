...
<head>
    ...
    <script type="text/javascript" src="${base}/script/w/sendTransPassCode.js"></script>
...
<form action="${base}/user/index!saveTranPass.htm" id="validateForm" method="post"/>
<div class="order-item">
    <ul>
     <li><label for="" class="lab-title">动态码</label><input type="tel" name="tradeAccount.tranPassword" validate="{required:true}" size="4" class="order-text1" placeholder="动态码"/>
          <input type="button"  onclick="sendCode2()" id="sendCodeBtn" class="btn order-btn1" value="获取动态码"/>
        </li>
        <#-->li><label for="" class="lab-title">旧密码</label><input type="password" name="tradeAccount.tranPassword" validate="{required:true}" size="18" class="order-text" placeholder="旧密码"/></li-->
