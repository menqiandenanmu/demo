...
<script language="JavaScript">
    $(document).ready(function(){
     	var fm=new BackUrlForm("validateForm","修改页面区域","${base}/manage/web/pageArea.htm");
    });
</script>
</head>
<body>
  <!--表单正文开始-->
            <div class="page-main">
                <div class="page-item page-title">修改页面区域</div>
                <div class="page-item form-panel">
                   <form action="${base}/manage/web/pageArea!update.htm" id="validateForm" method="post" >
                    <input name="id" type="hidden" value="${area.id}" />
                        <ul>
                             ...
                             <li><span class="lab-title">说明：</span>
                                <div class="form-item">
                                   <textarea name="area.info" cols="45" rows="5" class="textarea_1" id="textarea" validate="{maxlength:1000}">${area.info}</textarea>
                                </div>
                            </li>
                              <li><span class="lab-title">显示方式：</span>
                                <div class="form-item">
					              <select name="area.showFlag">
					          		<option value="0" <#if area.showFlag==0>selected</#if>>文本</option>
					          		<option value="1" <#if area.showFlag==1>selected</#if> >图片</option>
					          	</select>
                                </div>
                            </li>
                            ...
                            <li><span class="lab-title">内容条数：</span>
                                <div class="form-item">
                                    <input name="area.len" cols="45" rows="5" class="text" id="area.len" validate="{required:true,digits:true,min:1}" value="${area.len}"/>
                                </div>
                            </li>
                        </ul>
