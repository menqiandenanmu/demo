...
public class JdbcScrollPage extends Page // implements Page
{
	 ...
	 * 
	 * @param rs
	 *            ResultSet对象，ResultSet必须是可滚动的数据集，可以通过以下的Statement执行查询。 Statement stmt = conn.createStatement(
	 ...
	 * @throws SQLException
	 */
	public JdbcScrollPage(ResultSet rs, RowMapper rowMapper, int pageNumber,
			int pageSize) throws SQLException {
		super(pageNumber, pageSize, moveToLast(rs));
		this.result = extractDataList(rs, rowMapper, pageNumber, pageSize);
	}

	public JdbcScrollPage(ResultSet rs, int totalCount, RowMapper rowMapper,
	...
	public JdbcScrollPage(ResultSet rs, int totalCount, RowMapper rowMapper,
			PageRequest p) throws SQLException {
		this(rs, totalCount, rowMapper, p.getPageNumber(), p.getPageSize());
	}

	public JdbcScrollPage(ResultSet rs, RowMapper rowMapper, PageRequest p)
			throws SQLException {
	...
	private List extractDataList(ResultSet rs, RowMapper rowMapper,
			int pageNumber, int pageSize) throws SQLException {
