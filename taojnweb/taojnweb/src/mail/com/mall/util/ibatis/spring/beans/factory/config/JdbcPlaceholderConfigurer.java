package com.mall.util.ibatis.spring.beans.factory.config;
 ...
 * </pre>
 * 
 * [default_value]列可选 ?号为传递的placeholder的值
 * 
 * @author caedmon
 * 
 */
public class JdbcPlaceholderConfigurer extends PropertyPlaceholderConfigurer
		...
		if (value == null) {
			try {
				...
				throw new BeanDefinitionStoreException(
						"resolve placeholder by jdbc sql error,placeholder="
								+ placeholder, e);
			}
		}
		return value;
	...
	private String resolveJdbcPlaceholder(String placeholder) {
		SqlRowSet rs = new JdbcTemplate(dataSource).queryForRowSet(sql,
				new Object[] { placeholder });
		int columnCount = rs.getMetaData().getColumnCount();
		Assert
				.isTrue(
						columnCount == 1 || columnCount == 2,
						"error resultSet columnCount,sql example: select config_value,[default_value] from app_config where config_key=?,[default_value] is option");
		if (!rs.next())
			return null;
		...
		case 2:
			...
			return value;
		default:
			throw new IllegalArgumentException("error columnCount:"
					+ columnCount + ", sql:" + sql);
		}
	}
